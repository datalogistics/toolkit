...
int find_latlon(ServerConfig server, char *location, int *lat1, int *lon1) {
  ...
  char	*zip, *city, *state, *airport, *country;
  int	i, count, retval;
...
int get_latlon(ServerConfig server, double *lat, double *lon, char *zip, char *airport, char * city, char *state, char *country) {
  ...
  if ((city != NULL) && (country != NULL)) {
    if (strlen(country) == 4) {                /* make sure it is the correct length */
      ...
      for (i = 0; i < 3; i++) {
        ...
        fprintf(stderr, "  Location '%s' not found in ou=airports\n", filter);
        if (i == 0) {
          temp[strlen(filter) - 2] = '\0';
          strcat(temp, "*))");
        } else if (i == 1) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        fprintf(stderr, "  Location '%s' not found in ou=locations\n", filter);
        if (i == 0) {
          temp[strlen(filter) - 2] = '\0';
          strcat(temp, "*))");
        } else if (i == 1) {
  ...
  /* city not found, simply look for country */
  if (country != NULL) {
    if (strlen(country) == 4) {               /* make sure it is the correct length */
