    ...
    while (li_read < pi_size) {
        ...
        if (li_n <= 0) {
            ...
            for (li_inv = (li_n - 1); li_inv >= 0; li_inv--) {
                if ((*(lc_ptr + li_read + li_inv) == '\0')
                    || (*(lc_ptr + li_read + li_inv) == '\n')) {
...
long ReadComm(int pi_fd, char *pc_buf, size_t pi_size)
{
    ...
    while ( /*******(*(lc_ptr+li_read) != '\n') &&****/ (li_read < pi_size)) {
        li_n = read(pi_fd, lc_ptr + li_read, 1);
        if (li_n <= 0) {
            if ((errno == EINTR) || (errno == EINPROGRESS))
                continue;
            else if (errno == 0) {
...
int Socket2Queue(int pi_sockfd,
                 ...
                 ulong_t pl_fifoSize,
                 ulong_t pl_wrcount,
                 ulong_t * pl_written, int pi_HeaderLen, time_t pl_expireTime)
{
...
/* return -- IBP_OK or error number                */
/*****************************/
int File2Socket(FILE * pf_fp, int pi_sockfd, ulong_t pl_size,
...
int serve_known_socket(char *hostName, int port)
{
    ...
    do {
        ...
        setsockopt(sockId, SOL_SOCKET, SO_REUSEADDR, (char *) (&sockOpt),
                   sizeof(int));
...
# endif
...
#ifdef IBP_DEBUG
        fprintf(stderr,
                "SocketConnect(), gethostbyname () failed\n\terror: %d, %s\n",
                errno, strerror(errno));
#endif
        return -1;
    }
    ...
    case AF_INET:
        ...
        ls_addr4.sin_port = htons((short) port);
...
# ifdef AF_INET6
    case AF_INET6:
        ...
        ls_addr6.sin6_port = htons((short) port);
...
# ifdef IBP_DEBUG
        fprintf(stderr,
                "SocketConnect(), first GETFL fcntl() failed\n\terror: %d, %s\n",
                errno, strerror(errno));
# endif
        return -1;
    }
...
# ifdef IBP_DEBUG
        fprintf(stderr,
                "SocketConnect(), SETFL fcntl() failed\n\terror: %d, %s\n",
                errno, strerror(errno));
# endif
        return -1;
    }
    ...
    case AF_INET:
        li_ret =
            connect(li_sockfd, (struct sockaddr *) &ls_addr4,
                    sizeof(ls_addr4));
        break;
# ifdef AF_INET6
    case AF_INET6:
        li_ret =
            connect(li_sockfd, (struct sockaddr *) &ls_addr6,
                    sizeof(ls_addr6));
        break;
# endif
    default:
    ...
    if (li_ret < 0) {
        ...
        if ((li_ret =
             select(li_sockfd + 1, &ls_rset, &ls_wset, NULL,
                    lp_timer)) == 0) {
            close(li_sockfd);
            errno = ETIMEDOUT;
            return -1;
...
#ifdef IBP_DEBUG
            fprintf(stderr,
                    "SocketConnect (), getpeername () failed\n\terror %d, %s\n",
                    errno, strerror(errno));
#endif
            close(li_sockfd);
            return -1;
...
#ifdef IBP_DEBUG
        fprintf(stderr,
                "SocketConnect (), second GETFL fcntl() failed\n\terror: %d, %s\n",
                errno, strerror(errno));
#endif
        close(li_sockfd);
        return -1;
...
void handle_error(int nerr, void *errinfo)
{
    ...
    case E_USAGE:
        ...
        fprintf(stderr, "   -l  <lifetime>        : the max duration\n");
        fprintf(stderr,
                "   -p  <port>            : the port to use 1025-65534\n");
        fprintf(stderr,
                "   -pw <password>        : the password (to access and change the server fundamental data)\n");
        fprintf(stderr,
                "   -cf <cfg path>        : the path to find the config file and store the log file\n");
        fprintf(stderr,
                "   -c  <cfg name>        : the name of the config file\n");
        fprintf(stderr,
                "   -hn <hostname>        : the host name of the ibp server\n");
        fprintf(stderr,
                "   -tn <threads>         : number of worker threads to start initially.\n");
        fprintf(stderr,
                "   -tm <threads>         : limit of total number of worker threads running. (maximum is %d )  \n",
                MAX_THREADS);
        fprintf(stderr,
                "   -tl                   : enable per thread logging\n");
        fprintf(stderr, "   -nr                   : skip recovery\n");
        fprintf(stderr,
                "   -rt                   : the time of grace period\n");
        fprintf(stderr,
        ...
        fprintf(stderr,
                "In case some values will be missing from the IBP configuration file, IBP default values will be used. Please refer to the IBP documentaton for a complete explanation.\n");
    ...
    case E_PORT:
        if (ls_in->text1 != NULL) {
            fprintf(stderr,
                    "the port asked, %d, is out of range. The default IBP DATA port will be used\n",
                    *((int *) errinfo));
        }
        break;
    case E_ALLOC:
