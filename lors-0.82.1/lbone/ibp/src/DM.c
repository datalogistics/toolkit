...
Dlist array2Dlist(char **argarray, int argCount)
{
  ...
  /**  for(i=0;i<argCount;i++){
    ...
    for (i = 0; i < argCount; i++) {
        ...
        tempAN->args =
            (char **) malloc(sizeof(char *) * ((numTargets * 2) + 1));
        if (!strcmp(argarray[i], "+s")) {       /* source cap node */
            tempAN->type = S;
            i++;
            tempAN->args[0] =
                (char *) malloc(sizeof(char) * (strlen(argarray[i]) + 1));
...
char *BuildSourceCap(DMAS * dmas)
{
    ...
    sprintf(theCap, "ibp://%s:%d/%s/%d/READ", thisHost.nodename, IBPport,       /* dmas->ibp_port, */
            dmas->sourcekey, dmas->src_key);
...
int RunDMClient(DMASP * dmas, int ntargets, int dmtype, char *path)
{
    ...
    case DM_MBLAST:
        ...
        break;
    case DM_MULTICAST:
        sprintf(command,"%s%s",glbExecPath,"clientMCAST");
        args[0] = command;
        break;
        /*
    case DM_SABUL:
        sprintf(command,"%s%s",glbExecPath,"SabulSendfile");
        args[0] = command;
        break;
        */
    default:
...
int RunDMServer(DMAS * dmas, int dmtype, char *IBP_cap)
{
    ...
    char command[MAX_COMM_LEN], tempp[5], temps[5], tempf[64],tempsource[64];
    char *args[6];
    char mgroup[20] = "233.13.122.251";
   ...
   case DM_SABUL:
   case DM_SBL:
      sprintf(command,"%s%s",glbExecPath,"SabulRecvfile");
      args[0] = command;
      strcpy(tempsource,dmas->source);
      args[1] = tempsource ;
      sprintf(tempp,"%d",dmas->dm_port);
      args[2] = tempp;
      sprintf(tempf,"%ld",dmas->filesize);
...
int RequestToTarget(DMAS * dmas, Dlist argList, int t_id)
{
    ...
    sprintf(dest_caps.writeCap,
            "ibp://%s:%d/%s/%d/WRITE",
            dmas->target, dmas->ibp_port, dmas->targetkey, dmas->type_key);
...
int main(int argc, char *argv[])
{
    ...
    if (service == DM_SERVER) {
        ...
        sleep(MAX_WAIT_TIME);
        retval = RunDMClient(clientdmas, numTargets, dmtype, argv[2]);  /* create the client */
        /* fprintf(stderr,"---> retval RunDMClient=%d\n",retval); */

        while (attempts < MAX_NUM_ATTEMPTS) {
            if (retval < 0) {
                attempts++;
                fprintf(stderr, "Waiting for the Server, retry #%d...",
                        attempts);
                sleep(MAX_WAIT_TIME);
                retval = RunDMClient(clientdmas, numTargets, dmtype, argv[2]);  /* create the client */
            }
