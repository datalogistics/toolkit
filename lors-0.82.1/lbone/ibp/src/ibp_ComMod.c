...
int WriteData(IBP_CommUnit_t * ps_comm_unit, time_t pl_expireTime)
{
    ...
    retVal =
        write_data(ps_comm_unit->fd, ps_comm_unit->data_size,
                   ps_comm_unit->data_buf, pl_expireTime);
...
int ReadVarData(IBP_CommUnit_t * ps_comm_unit, time_t pl_expireTime)
{
    unsigned long int retVal;
    retVal =
        read_var_data(ps_comm_unit->fd, ps_comm_unit->data_size,
                      ps_comm_unit->data_buf, pl_expireTime);
    if (retVal == 0 && IBP_errno != IBP_OK)
        return (IBP_errno);
    ps_comm_unit->data_size = retVal;
...
int ReadFixData(IBP_CommUnit_t * ps_comm_unit, time_t pl_expireTime)
{
    unsigned long int retVal;
    retVal =
        read_fix_data(ps_comm_unit->fd, ps_comm_unit->data_size,
                      ps_comm_unit->data_buf, pl_expireTime);
    if (retVal == 0 && IBP_errno != IBP_OK)
        return (IBP_errno);
    ps_comm_unit->data_size = retVal;
    ...
    while (!d_EmptyCommSession(ps_comm_se)) {
        ...
        if (ls_comm_unit != NULL) {
            ...
            if ((lf_handlefunc = ls_comm_unit->handle_func) != NULL) {
                if (lf_handlefunc
                     (ps_comm_se, ls_comm_unit->data_buf,
                      ls_comm_unit->data_size) != IBP_OK)
                    return (NULL);
            }
...
int FillCommUnit(IBP_CommSession * ps_comm_se, 
                 ...
                 int pi_fd, char *pp_buffer, int pi_size,
                 DataType_t pe_datatype, func_pt * pf_func)
{
...
int HandleStoreResp(IBP_CommSession * ps_comm_session, 
                    char *pc_buf, unsigned long int pl_size)
{
    int li_resp;
...
/*****************************/
/* HandleMcopyResp           */
/* It handles the return of IBP_Mcopy()     */
/* return -- IBP_OK or error number                */
/*****************************/
int HandleMcopyResp(IBP_CommSession * ps_comm_session,
                   char *pc_data, unsigned long int pl_size)
{
    IBP_CommUnit_t * lp_comm_unit;
    int li_response;
