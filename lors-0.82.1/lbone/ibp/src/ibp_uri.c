    ...
    IURL * createIURL(char *capString, int mode, int timeout)
{
    ...
    if (lp_fields[3] == NULL || (lp_fields[4] == NULL)
          || (lp_fields[5] == NULL)) {
        IBP_errno = IBP_E_WRONG_CAP_FORMAT;
        free(lc_tmp);
        return (NULL);
    ...
    case IBP_CAP_TYPE_MANAGE:
        if (strcmp("MANAGE", lp_fields[5]) != 0
             && strcmp("READ", lp_fields[5]) != 0
             && strcmp("WRITE", lp_fields[5]) != 0) {
    ...
    if (sscanf(lp_fields[1], "%d", &(iurl->port)) != 1) {
         ...
         */ 
        if ((li_return =
             check_depot(iurl->hostname, &(iurl->port))) != IBP_OK) {
        IBP_errno = li_return;
        free_IURL(iurl);
        free(lc_tmp);
        return (NULL);
    ...
    if ((iurl->fd =
           connect_socket(iurl->hostname, iurl->port, timeout)) == -1) {
        free_IURL(iurl);
        free(lc_tmp);
        return (NULL);
    }
