...
/*! \def IBP_MAX_HOSTNAME_LEN
	\brief A definition defines the max length of the host name (256 bytes).

	The max length of host name is 256 bytes.
*/
...
typedef struct ibp_dptinfo
{
    ...
    char VersionMajor[IBP_MAX_VERSION_LEN];
    char VersionMinor[IBP_MAX_VERSION_LEN];
}
*IBP_DptInfo;
...
unsigned long int IBP_store(IBP_cap pc_cap, IBP_timer ps_timeout,
                            char *pc_data, unsigned long int pl_size);
unsigned long int IBP_remote_store(IBP_cap pc_cap,
                                   IBP_depot ps_remoteDepot,
                                   IBP_timer ps_timeout,
                                   unsigned long int pl_size);
unsigned long int IBP_load(IBP_cap pc_source, IBP_timer ps_timeout,
                           char *pc_buf,
                           unsigned long int pl_size,
                           unsigned long int pl_offset);
unsigned long int IBP_copy(IBP_cap ps_source, IBP_cap ps_target,
                           IBP_timer ps_sourceTimeout,
                           IBP_timer ps_targetTimeout,
                           unsigned long int pl_size,
                           unsigned long int pl_offset);
unsigned long int IBP_mcopy(IBP_cap pc_SourceCap, IBP_cap pc_TargetCap[],
                  unsigned int pi_CapCnt, IBP_timer ps_src_timeout,
                  IBP_timer ps_tgt_timeout, unsigned long int pl_size,
                  unsigned long int pl_offset, int dm_type[], int dm_port[],
                  int dm_service);
unsigned long int IBP_datamover(IBP_cap pc_TargetCap, IBP_cap pc_ReadCap,
                      IBP_timer ps_tgt_timeout, unsigned long int pl_size,
                      unsigned long int pl_offset, int dm_type, int dm_port,
                      int dm_service);
int IBP_manage(IBP_cap pc_manCap, IBP_timer ps_timeout,
               int pi_cmd, int pi_capType, IBP_CapStatus ps_info);
IBP_DptInfo IBP_status(IBP_depot ps_depot, int pi_StCmd,
                       IBP_timer ps_timeout, char *pc_password,
