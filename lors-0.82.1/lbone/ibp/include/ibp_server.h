...
typedef struct
{
    ...
    int NextIndexNumber;        /* Index number used for backup */
...
RequestQueue *queue;            /* IBP client request queue */

enum config
{ VOLSIZE = 1, STABLESIZE, STABLEDIR, VOLDIR, MAXDURATION, CFGPORT }
config;
...
void handle_store(int pi_fd, int pi_destfd, int pi_nbt,
                  IBP_byteArray ps_ByteArray);
void handle_lsd(int pi_fd, int pi_nbt, IBP_byteArray ps_ByteArray);
char *handle_manage(int pi_fd, int pi_nbt, IBP_byteArray ps_ByteArray);
