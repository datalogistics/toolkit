...
int curl_get_json_string(char *url, char **buf, longlong *size){
	...
	if(url == NULL){
		fprintf(stderr, "url not specified to get Json string \n");
		return LORS_FAILURE;
	}
...
int curl_post_json_string(char *url, char *buf, longlong size, char **response, longlong *len){
	...
	if(buf == NULL){
		fprintf(stderr, "No Json string to post \n");
		return LORS_FAILURE;
	}

	if(url == NULL){
		fprintf(stderr, "URL is not specified to post Json string \n");
		return LORS_FAILURE;
	}

	if(size <= 0){
		fprintf(stderr, "Buffer size is zero or less than zero \n");
		return LORS_FAILURE;
	}
	...
	if(res != CURLE_OK) {
		fprintf(stderr, "curl_handle global_init failed \n");
		return LORS_FAILURE;
	}

	curl_handle = curl_easy_init();	
	if(curl_handle == NULL){
		fprintf(stderr, "curl_handle easy init failed \n");
		return LORS_FAILURE;
	}
